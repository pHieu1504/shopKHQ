...
?>
 <form method="POST" action="modules/quanlysp/xuly.php?idsanpham=<?php echo $row['id_sanpham'] ?>" enctype="multipart/form-data">
	...
	<div class="input1">
	   <h3>Giảm Giá</h3>
	   <input type="text" value="<?php echo $row['sale'] ?>" name="sale">
	</div>

	<div class="input1">
	   <h3>Nơi Sản Xuất</h3>
	   <input type="text" value="<?php echo $row['sanxuat'] ?>" name="sanxuat">
	</div>
	<div class="input1 " >
	   <h3>Hình ảnh</h3>
	<div class="btn-ha">
		...
		<div class="custom-select" style="width:200px;">
			...
			<?php
	    		...
	    		while($row_band = mysqli_fetch_array($query_band)){
	    			if($row_band['id_band']==$row['id_band']){
	    		?>
	    		<option selected value="<?php echo $row_band['id_band'] ?>"><?php echo $row_band['tenband'] ?></option>
	    		<?php
	    			}else{
	    		?>
	...
	<input type="submit"  name="suasanpham" value="Sửa sản phẩm"></div>
